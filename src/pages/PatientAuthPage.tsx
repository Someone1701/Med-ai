...
const PatientAuthPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        const err = signIn(email, password, "patient");
        if (err) { toast.error(err); return; }
        toast.success("Welcome back!");
        navigate("/patient");
      } else {
        if (!displayName.trim()) { toast.error("Please enter your name"); return; }
        const err = signUp(email, password, displayName, "patient");
        if (err) { toast.error(err); return; }
        ...
        <div className="w-full max-w-md">
          <div className="bg-card rounded-2xl shadow-[var(--shadow-elevated)] p-8 relative overflow-hidden">
            ...
            <div className="text-center mb-8">
              ...
              <h1 className="text-3xl font-extrabold gradient-text">Patient {isLogin ? "Login" : "Sign Up"}</h1>
              <p className="text-muted-foreground mt-1">
                {isLogin ? "Sign in to view your medication schedule" : "Create an account to track your health"}
              </p>
            </div>
